"""Four-mode measurement-based non-Gaussian Floquet reservoir."""
...
class MBFloquetConfig:
    ...
    def __post_init__(self):
        ...
        if self.evolution not in {"unconditional", "conditional"}:
            raise ValueError("evolution must be unconditional or conditional")


class MBFloquetReservoir:
    """Projected-unitary fresh-mode encoding, collision, memory Floquet, and loss."""

    def __init__(self, config: MBFloquetConfig | None = None):
        self.config = config or MBFloquetConfig()
        c = self.config
        ...
        self._outcome_rows = tuple(
            np.asarray([self._joint_index.get(state + (outcome,), -1) for state in self.basis])
    ...
    def collision_kraus(self, value: float):
        if not -1 <= value <= 1:
            raise ValueError("CV-MB input must lie in [-1,1]")
        alpha = self.config.resource_strength + self.config.input_scale * value
        phase = np.exp(-1j * alpha * self._prep_values)
        joint_columns = self._collision_prep_left @ (phase[:, None] * self._collision_prep_right)
        operators = []
    ...
    def _features(self):
        ...
        for left in range(self.config.memory_modes):
            for right in range(left, self.config.memory_modes):
                value = float(probabilities @ (occupations[:, left] * occupations[:, right]))
    ...
    def step(self, value: float):
        ...
        self.time += 1
        diagnostics = state_diagnostics(self.state, self.basis, cutoff=self.config.cutoff)
        ...
        return {
            ...
            "provenance": {
                ...
                "backend": "manual total-Fock channel adapter cross-checked against PhotoGraphiQ 0.3.1",
                "non_gaussian_mechanism": "deterministic onsite n(n-1) Kerr kick",
                "postselection": False,
                "fresh_measured_mode": self.config.memory_modes,
                "photographiq_scope": "resource/measurement reference and independent backend checks",
            },
        }

    def non_gaussian_witness(self, value: float = 0.3):
        interacting = self.step(value)["features"]
        reference = MBFloquetReservoir(
            MBFloquetConfig(**{**asdict(self.config), "interaction": 0.0})
        ).step(value)["features"]
        return {
