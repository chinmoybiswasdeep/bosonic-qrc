...
def cutoff_convergence(
    ...
    comparisons = []
    for (_, previous, previous_rows), (cutoff, current, current_rows) in pairwise(models):
        ...
        if raw["convergence"].get("task_score_check", False):
            ...
            comparison["task_score_differences"] = {
                key: abs(prior_headline[key] - current_headline[key]) for key in prior_headline
            }
    ...
    for comparison in comparisons:
        checks = {
            ...
            "raw_trace": comparison["maximum_raw_trace_error"] <= thresholds["raw_trace_error"],
        ...
        if "maximum_task_score_difference" in comparison:
            checks["task_scores"] = (
                comparison["maximum_task_score_difference"] <= thresholds["task_score_difference"]
            )
...
def open_scan(
    ...
    for interaction in interactions:
        ...
        prototype = reservoir_class(_reservoir_config(config_class, open_raw, float(interaction)))
        channel = prototype.channel_diagnostics(open_raw["diagnostic_input"])
        echo_rng = np.random.default_rng(raw["seeds"]["reservoir"] + round(1000 * interaction))
        echo_curves = []
        ...
        if open_raw["run_tasks"]:
            for seed in open_raw["task_seeds"]:
                performances.append(
                    run_task_suite(_clone_factory(prototype), int(seed), protocol, input_domain)
                )
        if len(performances) == 1:
            ...
            performance = {
                "seeds": performances,
                "headline_mean": {
                    key: float(np.mean(values)) for key, values in headline_samples.items()
                },
                "headline_ci95": {
        ...
        if channel["subleading_eigenvalue_modulus"] < open_raw["overmixing_lambda2_threshold"]:
...
def _plots(output, closed, opened):
    ...
    if edge["estimate"] is not None:
        axis.axvline(edge["estimate"], color="black", linestyle="--", label="finite-size EOC")
...
def run_workflow(
    ...
    *,
    ...
    output = Path(output)
    output.mkdir(parents=True, exist_ok=True)
    closed_path = Path(closed_result) if closed_result else output / "closed_calibration.json"
    ...
    cutoff = cutoff_convergence(reservoir_class, config_class, raw, input_domain, protocol)
    ...
    checks = {
        ...
        "complete_channels_cptp": all(row["channel"]["validation_passed"] for row in opened),
        ...
        "echo_state_curves_recorded": all(row["echo_state"]["realizations"] >= 2 for row in opened),
    ...
    with (output / "cutoff_convergence.csv").open("w", newline="", encoding="utf-8") as handle:
        fields = [
            "cutoff",
    ...
    with (output / "open_results.csv").open("w", newline="", encoding="utf-8") as handle:
        fields = [
            "interaction",
            "d_eoc",
            "region",
...
def merge_chunks(paths, output):
    ...
    interactions = [value for row in manifests for value in row["chunk"]["interactions"]]
