...
def fixed_number_basis(modes: int, particles: int) -> tuple[tuple[int, ...], ...]:
    ...
    return tuple(
        state for state in product(range(particles + 1), repeat=modes) if sum(state) == particles
    )
...
def total_cutoff_basis(modes: int, cutoff: int) -> tuple[tuple[int, ...], ...]:
    ...
    return tuple(state for state in product(range(cutoff), repeat=modes) if sum(state) < cutoff)
...
def exact_action(hamiltonian: np.ndarray, state: np.ndarray, interval: float) -> np.ndarray:
...
def strang_unitary(parts: HamiltonianParts, interval: float, substeps: int) -> np.ndarray:
    if substeps < 1:
...
def reflection_blocks(
    ...
    if minus:
        blocks["reflection_odd"] = np.asarray(minus).conj() @ matrix @ np.asarray(minus).T
    return blocks
...
def circular_phase_ratios(unitary: np.ndarray) -> np.ndarray:
    ...
    return np.minimum(spacings, np.roll(spacings, -1)) / np.maximum(spacings, np.roll(spacings, -1))


def spectral_form_factor(spectra: list[np.ndarray], times: np.ndarray) -> dict[str, np.ndarray]:
    """Ensemble averaged unconnected and connected spectral form factors."""
    times = np.asarray(times, float)
    traces = np.asarray(
        [[np.sum(np.exp(-1j * time * spectrum)) for time in times] for spectrum in spectra]
    )
...
def reduced_density_matrix(
    ...
    for row, bra in enumerate(basis):
        ...
        for column, ket in enumerate(basis):
            ...
            result[reduced_index[bra_keep], reduced_index[ket_keep]] += density[row, column]
...
def coherent_amplitudes(alpha: complex, cutoff: int) -> np.ndarray:
    values = np.asarray(
        [np.exp(-(abs(alpha) ** 2) / 2) * alpha**n / np.sqrt(factorial(n)) for n in range(cutoff)],
        complex,
    )
