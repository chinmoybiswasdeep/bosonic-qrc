...
def delayed_parity(length: int, seed: int, delay: int = 3):
    ...
    target = np.asarray([np.sum(bits[index : index + delay + 1]) % 2 for index in range(length)])
...
def nonlinear_channel(length: int, seed: int, snr_db: float = 20):
    ...
    for time in range(2, length + 5):
        linear[time] = sum(
            coefficient * symbols[time + offset] for offset, coefficient in weights.items()
        )
...
def mackey_glass(length: int, seed: int, tau: int = 17):
    ...
    for time in range(tau, len(values) - 1):
        ...
        values[time + 1] = values[time] + (0.2 * delayed / (1 + delayed**10) - 0.1 * values[time])
...
def rff_features(inputs: np.ndarray, dimension: int, seed: int):
    ...
    return np.sqrt(2 / dimension) * np.cos(
        inputs[:, None] * rng.normal(size=dimension) + rng.uniform(0, 2 * np.pi, dimension)
    )
...
def ipc_targets(inputs: np.ndarray, maximum_delay: int, maximum_degree: int):
    ...
    span = np.ptp(values)
    scaled = np.zeros_like(values) if span == 0 else 2 * (values - values.min()) / span - 1
    ...
    for delay in range(1, maximum_delay + 1):
        for degree in range(1, maximum_degree + 1):
            ...
            targets[label] = np.polynomial.legendre.legval(delayed[:, delay], coefficient)
    for left in range(1, maximum_delay):
        for right in range(left + 1, maximum_delay + 1):
            targets[f"degree=2;delays={left},{right}"] = delayed[:, left] * delayed[:, right]
    return targets


def effective_feature_rank(features: np.ndarray, relative_threshold: float = 1e-10) -> int:
