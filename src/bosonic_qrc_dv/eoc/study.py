...
from .collision import CollisionConfig, RecurrentDVReservoir
from .core import (
    bose_hubbard_parts,
    density_fidelity,
    exact_unitary,
...
def _memory_score(features, inputs):
    ...
    coefficients = np.linalg.pinv(standardized[train]) @ (target[train] - target[train].mean())
...
def _open_scan(raw, boundaries):
    ...
    for interaction in raw["open_system"]["interactions"]:
        ...
        settings = CollisionConfig(
            memory_modes=modes,
            local_cutoff=raw["open_system"]["cutoff"],
            hopping=1.0,
            interaction=float(interaction),
            disorder=(0.0,) * (modes + 1),
            interval=raw["open_system"]["interval"],
            memory_transmissivity=raw["open_system"]["transmissivity"],
            seed=raw["seeds"]["reservoir"],
        )
        first = RecurrentDVReservoir(settings)
        second = RecurrentDVReservoir(settings)
        features = []
        distances = []
        ...
        nearest = min(boundaries, key=lambda item: abs(interaction - item)) if boundaries else None
        rows.append(
            {
                "interaction": float(interaction),
                ...
                "boundary_shell_population": diagnostics.get("boundary_shell_population"),
                "mean_photon_number": diagnostics["mean_photon_number"],
                "maximum_supported_photon_number": diagnostics["maximum_supported_photon_number"],
                **first.channel_diagnostics(0.5),
            }
        )
    return rows
...
def _align_density(density, source_basis, destination_basis):
    lookup = {state: index for index, state in enumerate(destination_basis)}
    ...
    for row, bra in enumerate(source_basis):
        for column, ket in enumerate(source_basis):
            aligned[lookup[bra], lookup[ket]] = density[row, column]
    return aligned
...
def _cutoff_convergence(raw):
    ...
    models = []
    interaction = raw["open_system"]["interactions"][len(raw["open_system"]["interactions"]) // 2]
    modes = raw["open_system"]["memory_modes"]
    ...
    for (_, previous, _), (cutoff, current, last) in pairwise(models):
        union = tuple(sorted(set(previous.memory_basis) | set(current.memory_basis)))
        left = _align_density(previous.state, previous.memory_basis, union)
        right = _align_density(current.state, current.memory_basis, union)
        rows.append(
            {
                "cutoff": cutoff,
                "trace_distance_to_previous": trace_distance(left, right),
                "fidelity_to_previous": density_fidelity(left, right),
                "boundary_population": last["diagnostics"]["local_cutoff_boundary_population"],
            }
        )
    return rows
...
def run(
    ...
    *,
    ...
    if chunk_count < 1 or not 0 <= chunk_index < chunk_count or chunk_count > len(interactions):
        raise ValueError("Require 0 <= chunk-index < chunk-count <= open interaction count")
    chunk = {"index": chunk_index, "count": chunk_count}
    if resume and (manifest := _resume_manifest(output, config_hash, chunk)) is not None:
        return manifest
    ...
    (output / "gate_report.json").write_text(json.dumps(gate, indent=2), encoding="utf-8")
    ...
    manifest = {
        ...
        "dependencies": {
            ...
            for package in ("numpy", "scipy", "matplotlib", "PyYAML", "perceval-quandela")
        },
    }
    checksums = {}
