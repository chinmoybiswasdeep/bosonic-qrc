...
def rank_diagnostics(
    ...
    participation = float(squared.sum() ** 2 / np.sum(squared**2)) if np.any(squared) else 0.0
...
def independent_null_capacities(
    ...
    for index in range(count):
        ...
        validation = evaluate_target(rng.uniform(-1, 1, len(validation_features) + delay), target)
...
def target_bank_diagnostics(targets: np.ndarray) -> dict[str, object]:
    ...
    effective = (
        float(eigenvalues.sum() ** 2 / np.sum(eigenvalues**2)) if np.any(eigenvalues) else 0.0
    )
