...
class FloquetChaosConfig:
    ...
    def __post_init__(self):
        ...
        if any(modes < 3 or particles < 1 for modes, particles in self.sizes):
            raise ValueError(
                "each closed size needs >=3 modes and positive particle number"
            )
...
def floquet_parts(
    ...
    for column, state in enumerate(basis):
        ...
        h_u[column, column] = (
            0.5 * interaction * np.sum(occupation * (occupation - 1))
            + phases @ occupation
        )
        for left, right in _edges(modes, boundary):
            for source, destination in ((left, right), (right, left)):
                ...
                if row is not None:
                    h_j[row, column] += -hopping * np.sqrt(
...
def sff_ensemble(
    unitaries: list[np.ndarray], steps: int, rng, bootstrap_replicates: int, cls: str
):
    ...
    return {
        ...
        "reference_class": cls,
        "random_matrix_reference": random_matrix_sff(
            times, len(unitaries[0]), cls
        ).tolist(),
        "thouless_time": None,
...
def eigenvector_diagnostics(unitary: np.ndarray, basis):
    ...
    for column in selected:
        ...
        reduced, _ = reduced_density_matrix(
            np.outer(vector, vector.conj()), basis, keep
        )
    ...
    return {
        ...
        "normalized_participation_entropy": float(
            np.mean(participation_entropy) / log(len(basis))
        ),
    ...
    for index in range(len(parameters) - 1):
        ...
        if left == 0:
            ...
            result.append(
                float(
                    parameters[index]
                    + weight * (parameters[index + 1] - parameters[index])
                )
            )
    return result
...
def _edge_estimate(size_rows, config: FloquetChaosConfig, rng):
    ...
    for size in size_rows:
        ...
        boot_edges = []
        per_point = [
            np.asarray(point["realization_mean_ratios"]) for point in size["points"]
        ]
        for _ in range(config.bootstrap_replicates):
            ...
            sampled_scores = np.clip(
                (np.asarray(sampled) - poisson) / (chaotic - poisson), 0, 1
            )
        ...
        size["entry_ci95"] = (
            _bootstrap_ci(boot_edges) if len(boot_edges) >= 10 else [None, None]
        )
    ...
    ci = (
        _bootstrap_ci(boot_extrapolated)
        if len(boot_extrapolated) >= 10
        else [None, None]
    )
    return {
...
def scan_floquet_chaos(config: FloquetChaosConfig):
        ...
        for interaction in config.interactions:
            ...
            if len(valid) != len(statistics):
                ...
                boot = [
                    np.mean(
                        root_rng.choice(
                            realization_ratios, len(realization_ratios), replace=True
                        )
                    )
                    for _ in range(config.bootstrap_replicates)
                ]
            ...
            points.append(
                {
                    ...
                    "quasienergies_by_realization": [
                        row["phases"].tolist() for row in statistics
                    ],
                    "spacing_ratios_by_realization": [
                        row["ratios"].tolist() for row in statistics
                    ],
                    "sff": sff_ensemble(
                        unitaries,
    ...
    if any(
        point["degeneracy_failure"] for size in size_rows for point in size["points"]
    ):
