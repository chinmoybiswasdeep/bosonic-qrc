...
def complete_channel_diagnostics(
    ...
    *,
    ...
    if dense:
        ...
        non_normality = float(
            np.linalg.norm(
                superoperator.conj().T @ superoperator
                - superoperator @ superoperator.conj().T
            )
            / normality_scale
        )
    else:
        ...
        scales = []
        for _ in range(6):
            probe = probe_rng.normal(size=dimension**2) + 1j * probe_rng.normal(
                size=dimension**2
            )
            differences.append(
                np.linalg.norm(adjoint(forward(probe)) - forward(adjoint(probe)))
            )
    ...
    if dense:
        ...
        vector = probe_rng.normal(size=dimension**2) + 1j * probe_rng.normal(
            size=dimension**2
        )
    ...
    *,
    ...
    for stage in stages:
        vectors = np.column_stack(
            [operator.reshape(-1, order="F") for operator in stage]
        )
        stage_gram_minima.append(
            float(np.linalg.eigvalsh(vectors.conj().T @ vectors).min())
        )
    ...
    initial = project(
        spectral_rng.normal(size=dimension**2)
        + 1j * spectral_rng.normal(size=dimension**2)
    )
    ...
    ritz_values, ritz_vectors = np.linalg.eig(
        hessenberg[:arnoldi_steps, :arnoldi_steps]
    )
    ...
    vector = project(
        probe_rng.normal(size=dimension**2) + 1j * probe_rng.normal(size=dimension**2)
    )
    vector /= np.linalg.norm(vector)
    contraction = 0.0
    for _ in range(12):
