...
def evaluate_target(factory, inputs, target, protocol: TaskProtocol):
    ...
    nmse = (
        float(np.mean((test_y - prediction) ** 2) / variance) if variance > 0 else 1.0
    )
    return {
        "test_nmse": nmse,
...
def run_task_suite(factory, seed: int, protocol: TaskProtocol, input_domain: str):
    ...
    results["delayed_parity"] = evaluate_target(
        factory, parity_drive, parity_target, protocol
    )
...
def summarize_against_chaos(rows):
    ...
    def samples(performance, metric):
        if "seeds" in performance:
            return np.asarray(
                [samples(seed, metric)[0] for seed in performance["seeds"]], float
            )
    ...
    for metric in metrics:
        ...
        summary[metric] = {
            ...
            "by_region": by_region,
            "crossover_minus_regular_cohens_d": effect_size(
                edge_samples, regular_samples
            ),
            "crossover_minus_chaotic_cohens_d": effect_size(
                edge_samples, chaotic_samples
            ),
