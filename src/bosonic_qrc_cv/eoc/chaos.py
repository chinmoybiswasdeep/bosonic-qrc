...
class ChaosScanConfig:
    ...
    def __post_init__(self):
        ...
        if self.boundary != "open":
            raise ValueError(
                "current symmetry resolver supports open reflection sectors only"
            )
    ...
    for left in range(len(score) - 1):
        if score[left] == 0:
            ...
            crossings.append(
                float(
                    parameters[left]
                    + fraction * (parameters[left + 1] - parameters[left])
                )
            )
...
def scan_closed_core(config: ChaosScanConfig) -> dict[str, object]:
    ...
    for interaction in config.interactions:
        ...
        eigenvalues = np.concatenate(
            [np.linalg.eigvalsh(block) for block in blocks.values()]
        )
        ...
        otoc = [
            infinite_temperature_otoc(
                exact_unitary(parts.total, time), local_left, local_right
            )
            for time in times
        ]
        rows.append(
            {
                ...
                "spacing_ratio_ci95": _bootstrap_mean(
                    ratios, rng, config.bootstrap_replicates
                ),
                ...
                "sector_dimensions": {
                    name: len(block) for name, block in blocks.items()
                },
            }
        )
    ...
    for row in rows:
        if crossings:
            distance = min(
                crossings, key=lambda crossing: abs(row["interaction"] - crossing)
            )
            sign = 1 if row["mean_spacing_ratio"] >= midpoint else -1
            row["signed_distance_to_nearest_boundary"] = sign * abs(
                row["interaction"] - distance
            )
        else:
    ...
    return {
        ...
        "spectral_form_factor": {
            key: value.tolist() for key, value in form_factor.items()
        },
        "times": times.tolist(),
