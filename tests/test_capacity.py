...
def test_multi_indices_are_deterministic_and_nonconstant():
    ...
    second = enumerate_targets(3, 4, 2)
    assert first == second
    assert all(item.degree == sum(item.multi_index) > 0 for item in first)
    assert all(
        item.maximum_delay
        == max(i for i, degree in enumerate(item.multi_index) if degree)
        for item in first
    )
...
def test_perfect_linear_delay_and_shuffled_control():
    ...
    definition = next(
        item for item in enumerate_targets(1, 2, 1) if item.maximum_delay == 2
    )
