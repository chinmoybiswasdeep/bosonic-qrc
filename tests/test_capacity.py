...
from bosonic_qrc_cv.capacity import (
    benjamini_hochberg,
    capacity_bound,
    enumerate_targets,
    evaluate_target,
...
from bosonic_qrc_cv.capacity import test_r2 as coefficient_of_determination
...
def test_multi_indices_are_deterministic_and_nonconstant():
    ...
    second = enumerate_targets(3, 4, 2, maximum_targets=20)
    assert first == second
    assert all(item.degree == sum(item.multi_index) > 0 for item in first)
    assert all(item.maximum_delay == max(i for i, degree in enumerate(item.multi_index) if degree) for item in first)
