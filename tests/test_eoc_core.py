...
from bosonic_qrc_cv.eoc.core import (
    bose_hubbard_parts,
    channel_spectrum,
...
def test_exact_sparse_floquet_and_strang_convergence():
    ...
    errors = [
        np.linalg.norm(strang_unitary(parts, 0.4, steps) - exact) for steps in (1, 2, 4)
    ]
...
def test_exact_kerr_phase_and_reflection_sector_resolution():
    ...
    expected = np.diag(
        [
            np.exp(-0.5j * 0.6 * interval * sum(n * (n - 1) for n in state))
            for state in basis
        ]
    )
