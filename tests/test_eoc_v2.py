...
from bosonic_qrc_cv.eoc.channel import complete_channel_diagnostics
from bosonic_qrc_cv.eoc.core import fixed_number_basis
from bosonic_qrc_cv.eoc.floquet_reservoir import (
    CVFloquetReservoir,
    _annihilation,
)
from bosonic_qrc_cv.eoc.performance import TaskProtocol, run_task_suite
from bosonic_qrc_cv.eoc.science import (
    floquet_parts,
    floquet_unitary,
    quasienergy_statistics,
    sff_ensemble,
)
from bosonic_qrc_cv.eoc.workflow import _coordinate, _verify_artifacts
...
def test_complete_four_mode_channel_loss_and_non_gaussianity():
    model = CVFloquetReservoir()
...
def test_real_task_runner_exercises_every_advertised_family():
    ...
    result = run_task_suite(CVFloquetReservoir, 211, protocol, "cv")
    assert len(result["advertised_tasks_exercised"]) == 7
    assert {"narma10", "mackey_glass", "nonlinear_channel_equalization"} <= set(
        result["tasks"]
    )
